...
@implementation NSString (AAPassword)

- (NSString *)aa_MD5Digest
{
...
//转化为Base64
+ (NSString *)aa_base64StringFromText:(NSString *)text
{
    if (text && ![text isEqualToString:LocalStr_None]) {
        ...
        //IOS 自带DES加密 End
        return [self aa_base64EncodedStringFrom:data];
    }
    else {
...
//由base64转化
+ (NSString *)aa_textFromBase64String:(NSString *)base64
{
    if (base64 && ![base64 isEqualToString:LocalStr_None]) {
        //取项目的bundleIdentifier作为KEY   改动了此处
//        NSString *key = [[NSBundle mainBundle] bundleIdentifier];
        NSData *data = [self aa_dataWithBase64EncodedString:base64];
...
//DES加密
+(NSString *)aa_DESEncryptSting:(NSString *)sText key:(NSString *)key andDesiv:(NSString *)ivDes
{
...
//DES解密
+(NSString *)aa_DESDecryptWithDESString:(NSString *)sText key:(NSString *)key andiV:(NSString *)iv
{
    if ((sText == nil || sText.length == 0)
        || (sText == nil || sText.length == 0)
        || (iv == nil || iv.length == 0)) {
...
//AES加密
+ (NSData *)aa_AES128EncryptWithKey:(NSString *)key iv:(NSString *)iv withNSData:(NSData *)data
{
...
//AES解密
+ (NSData *)aa_AES128DecryptWithKey:(NSString *)key iv:(NSString *)iv withNSData:(NSData *)data
{
    char keyPtr[kCCKeySizeAES128+1];
    bzero(keyPtr, sizeof(keyPtr));
...
/******************************************************************************
 ...
 备注信息 :
 ******************************************************************************/
+ (NSData *)aa_dataWithBase64EncodedString:(NSString *)string
{
    if (string == nil)
...
/******************************************************************************
 ...
 备注信息 :
 ******************************************************************************/
+ (NSString *)aa_base64EncodedStringFrom:(NSData *)data
{
    if ([data length] == 0)
        return @"";
